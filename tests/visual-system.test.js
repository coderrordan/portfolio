...
test('compact laptops keep desktop navigation from 1024px', async () => {
  ...
  assert.match(css, /@media \(min-width: 64rem\) and \(max-height: 50rem\)/)
  assert.match(css, /\.rail-index\s*\{[^}]*min-height:\s*0[^}]*overflow-x:\s*hidden[^}]*overflow-y:\s*auto/s)
})

test('final supporting typography and motion share one responsive system', async () => {
  const css = await read('../src/index.css')
  assert.match(css, /\.hero-sub,[\s\S]*?\.services-intro > p,[\s\S]*?\.cases-intro > p,[\s\S]*?\.os-description,[\s\S]*?\.cta-copy > p\s*\{[^}]*font-size:\s*clamp\(1\.08rem,\s*1\.25vw,\s*1\.25rem\)[^}]*line-height:\s*1\.65/s)
  assert.match(css, /\.cta-actions a\s*\{[^}]*width:\s*100%/s)
  assert.match(css, /\.terminal-box--embedded \.terminal-line\s*\{[^}]*font-size:\s*0\.75rem/s)
  assert.match(css, /\.reveal-ready \[data-reveal\]/)
  assert.match(css, /\[data-reveal='group'\]/)
  assert.match(css, /\[data-reveal='visual'\]/)
  assert.match(css, /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\[data-reveal\][^}]*opacity:\s*1\s*!important/s)
})
...
test('Europe map behaves as an unframed hero background', async () => {
  ...
  assert.match(css, /\.process-route li\s*\{[^}]*min-height:\s*clamp\(14rem/s)
...
test('services use a light editorial surface with readable ink', async () => {
  ...
  assert.match(css, /\.services-intro h2 strong\s*\{[^}]*color:\s*var\(--nexus-accent-primary\)/s)
